class Parser{
  ...
  static async parseCdiJsonLd(data, csvdata, doneCallback){
    var dataset = new Dataset()
    console.log("parseCdiJsonLd", data)
    dataset.data = data
    dataset.csvdata = csvdata
    // Serialize the dataset with pretty output
    dataset.serializedData = JSON.stringify(data, null, 2); // 2 spaces for indentation

    dataset.location = "remote"
    dataset.fileName = 'external_data.json'
    dataset.studyName = 'DDI-CDI demo dataset'
    dataset.studyGroupName = 'DDI-CDI demo dataset'
    dataset.mimeType = 'application/json'
    dataset.fileSize = data.length
    dataset.errors = []
    dataset.fileArrayBuffer = data
    dataset.lastModified = new Date().toISOString()
    //dataset.sha256 = await checksum(data, "SHA-256")
    dataset.inputType = 'json-ld'
    dataset.columns = []
    dataset.delimiter = null
    dataset.linebreak = null
    dataset.firstRowIsHeader = false
